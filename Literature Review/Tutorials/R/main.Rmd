...
In addition to the same checks we did on the response variable, we also going to encode the categorical features.
...
From *Resonse Variable Against Features Overview* section it seems that houses in *Residential Low Density* are the most expensive houses then *Floating Village Residential* and *Commercial* are the cheapest.

Let us investigate correlation.

```{r MSZoning}
cor(train$SalePrice, as.integer(train$MSZoning))
```

From above and *Corrleation* section it seems it is not a big contributor feature. I will drop it.
